...
def update_role_by_name(old_role_name, new_role_name):

    role_to_update = Role.query.filter_by(role_name=old_role_name).first()
    if role_to_update:
...
def delete_role_by_name(role_name):
    if(Role.query.filter_by(role_name=role_name).delete()):
        db.session.commit()
        return {'success':True}
    else:
        return {'success':False}